...
class DES (object):
    ...
    __S = partial(lambda hex_bin, __s, s: ''.join(hex_bin['%x' % __s[i][int(s[i*6] + s[i*6+5], 2)*16 + int(s[i*6+1:i*6+5],
                                                                               2)]] for i in range(8)), __hex_bin, __s)
    ...
    __K0 = partial(lambda k0, K2, k: map(K2, (k[k0[i]:28] + k[0:k0[i]] + k[k0[i]+28:56]+k[28:28+k0[i]] for i in range(16))), __k0, __K2)
